...
// Constructor to initialize library system and load data from database
...
// function to initialize the library system by loading all necessary data from the database
...
bool LibrarySystem::issueBook(const string &isbn, const string &sid)
{
    ...
    if (isbn.empty() && total && available)
    {
        qDebug() << "book data fetching failed";
        return false;
    }
